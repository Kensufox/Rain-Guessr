...
function parse_region() {
	...
	let region_map = "/World/Regions/" + region + "/map_" + region + ".txt";
	try {
	...
	region_map.on("line", function(line) {
		...
		if(line.substring(0, 3) === region + "_")
			room_map = readline.createInterface({ input: fs.createReadStream("/World/Regions/" + region + "/Rooms/" + map_name + ".txt") });
		else
			room_map = readline.createInterface({ input: fs.createReadStream("/World/Gates/" + map_name + ".txt") });
